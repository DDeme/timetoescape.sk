...
const Header = ({ showNav = true, isHome, changeBgOpacity = false }: Props) => {
  ...
  return (
    <HeaderContainer isOpen={isOpen} changeBgOpacity={changeBgOpacity} isScrolled={scrollTopPercentage > 0}>
      <NavContainer as={'nav'}>
        {isHome ? (
          <AnchorLink offset={offset} className="anchor" href={'#intro'} aria-label="Navigate to home" as={SidePart}>
            <LogoIcon />
          </AnchorLink>
        ) : (
