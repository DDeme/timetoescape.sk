...
export const CustomerCard = ({ customer }: props) => {
  const {
    ...
    query CustomerImages {
      allFiles: allFile(filter: { extension: { regex: "/jpg/" }, relativeDirectory: { eq: "reviews" } }) {
        edges {
          node {
            name
            id
  ...
  const gatsbyImage = images.find(({ node }: { node: FileNode }) => node?.relativePath === customer.customerImage)
    ...
    <Card tw="flex flex-1 flex-col p-5 md:p-10 ">
      <div tw="flex mb-8">
        <StarRatings
          ...
          name="hodnotenie"
        />
        <div tw="float-right flex-1 text-right">{showTextRating(customer.rating, customer.scale)}</div>
      </div>
      <div tw="flex-grow">
      ...
      <div tw="flex items-center mt-8">
        {image && <GatsbyImage image={image} alt={customer.customerName} tw="w-12 h-12 mr-4 rounded-full" />}
        <div>
