...
const TextWrapper = styled.div`
  ...
  background: radial-gradient(circle, rgba(33, 33, 33, 0.8) 25%, rgba(33, 33, 33, 0) 100%);
...
export const MainPageSection = ({
  ...
  const InsideContent = (
    <TextWrapper className={className}>
      <DividerWrapper hasDividerOnTop={hasDividerOnTop} hasDividerOnBottom={hasDividerOnBottom}>
        <Container>{children}</Container>
      </DividerWrapper>
    </TextWrapper>
  )

  const BackgroundVideo = videoEnabled ? loadable(() => import('./BackgroundVideo')) : Fragment
